...
Analysis}（ABSA），透過 Qwen 辨識留言正負面情緒所針對的主體或面向，以判斷負面來源究竟是內容問題、影片
呈現問題、合作對象問題，或是外部事件所帶來的聲譽風險。
...
的組成、回訪行為、社群結構、情緒反應、語意主體與互動風險，並進一步觀察外部輿論事件是否反映到 YouTube
留言生態。透過留言語意分析、ABSA、觀眾網路分析與 \eng{reply-thread conflict analysis}，本研究希望協助
...
還是風險」、「正負面留言究竟針對什麼」、「哪些觀眾群對哪些內容特別敏感」，以及「外部事件是否反映到 YouTube
留言生態」。這正是本研究希望透過留言語意分析、ABSA、觀眾網路與 reply-thread conflict analysis 補足的缺口。
...
\eng{YouTube-native audience intelligence}：也就是從留言者社群、留言語意、情緒主體、互動衝突與外部事件
反映等角度，判斷頻道互動的品質與長期價值。

具體而言，本研究結合影片 metadata、Qwen 情緒標註、ABSA、co-commenter network、reply-thread conflict
analysis 與外部輿論事件時間軸，建立一套 YouTube 頻道分析框架，主要回答以下五個研究問題：
...
\begin{description}[leftmargin=3.4em,style=nextline,itemsep=3pt]
  ...
  \item[\textbf{RQ5}] 外部輿論事件是否與 YouTube 留言量、負面率、ABSA 負面主體、reply conflict 或新留言者
        變化出現時間上的關聯跡象？
\end{description}
