...
本章依第~\ref{sec:method} 節的分析框架，逐一回答五個研究問題。除特別標明外，
結構與互動密度指標以主留言為口徑，情緒指標以含回覆的 Qwen 標註為口徑（見表~\ref{tab:scope}）。
...
\subsection{外部事件反映分析（RQ5）}
\label{sec:case-external}

本研究蒐集 8 個與 The DoDo Men 相關的外部事件叢集（來源 PTT／Dcard），並比較事件前後 YouTube 留言變化。
分析發現，多數事件後的「新留言者占比」並未\textbf{顯著高於}頻道長期基準——8 個事件中僅 2 個出現 $\ge 1$ 項
指標（留言量、負面率、衝突、新觀眾、外溢）顯著偏離平常。換言之，雖然事件後一定會有新留言者，但其占比多反映
頻道自然成熟，而非外部討論明確帶入新觀眾；少數真正高於基準的事件，新觀眾的後續回訪率也僅約 5\%，多為一次性。

\textbf{對 RQ5 的回答}：本研究僅觀察到\textbf{少數}外部事件與 YouTube 留言變化出現時間上的關聯跡象，且整體而言
外部討論並未顯著、穩定地為頻道帶入可長期經營的新留言者。此結果提醒，外部聲量不應被高估為穩定的新客來源；惟此
分析僅為關聯跡象、非因果推論（外部資料較稀疏，亦為限制之一）。
...
(2) conflict score 與圍剿／對立母串數、(3) 新留言者負面率、(4) 外部事件時間軸、(5) 各觀眾社群的情緒變化。
若多個風險指標同時升高（如負面被放大、衝突上升、外部聲量同步進入），應優先處理。本研究的個案顯示，The DoDo Men
