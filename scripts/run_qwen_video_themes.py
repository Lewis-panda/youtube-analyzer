...
from channel_analyzer.qwen_video import (
    DEFAULT_MODEL,
...
def build_parser() -> ArgumentParser:
    parser = ArgumentParser(description="Classify channel videos with Qwen.")
    ...
    parser.add_argument("--limit", type=int, default=None, help="Classify only N remaining videos.")
    parser.add_argument("--max-model-len", type=int, default=2048)
    parser.add_argument("--attention-backend", default="TRITON_ATTN")
    parser.add_argument("--gpu-memory-utilization", type=float, default=0.90)
    parser.add_argument(
        "--output",
        help="Optional CSV path. Default: runs/<slug>/tables/qwen_video_themes.csv",
    )
    parser.add_argument(
        "--repair-existing",
        action="store_true",
        help="Re-parse theme_raw in the existing output CSV and exit.",
    )
    parser.add_argument(
        "--retry-parse-errors",
        action="store_true",
        help="Drop existing parse-error rows before selecting remaining videos.",
    )
    return parser
...
def main() -> None:
    ...
    output_path = (
        ...
        if args.output
        else ROOT / "runs" / slug / "tables" / "qwen_video_themes.csv"
    )
    ...
    if args.repair_existing:
        ...
        parse_errors = int(repaired.get("theme_parse_error", False).sum())
        print(f"Repaired: {output_path}", flush=True)
        print(f"Rows: {len(repaired):,}", flush=True)
        print(f"Parse errors: {parse_errors:,}", flush=True)
        return

    videos = data.videos.copy().sort_values("published_at").reset_index(drop=True)
    if args.retry_parse_errors and output_path.exists() and output_path.stat().st_size > 0:
        ...
        if "theme_parse_error" in existing.columns:
            before = len(existing)
            existing = existing[existing["theme_parse_error"] != True].copy()
    ...
    if output_path.exists() and output_path.stat().st_size > 0:
        ...
        done_count = pd.read_csv(
            output_path, usecols=["video_id"], low_memory=False
        )["video_id"].nunique()
    remaining = select_remaining(videos, output_path, args.limit)
    print(f"Channel: {data.channel.get('title')} ({data.channel.get('channel_id')})", flush=True)
    print(f"Videos in scope: {len(videos):,}", flush=True)
    ...
    if remaining.empty:
        ...
        print(f"No remaining videos. Output: {output_path}", flush=True)
        return
    ...
    parse_errors = int(merged.get("theme_parse_error", False).sum())
